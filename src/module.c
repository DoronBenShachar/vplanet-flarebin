...
void InitializeModule(BODY *body, CONTROL *control, MODULE *module) {
  ...
  module->iaFlareBin      = malloc(iNumBodies * sizeof(int));
...
void AddModules(BODY *body, CONTROL *control, MODULE *module) {
  ...
  for (iBody = 0; iBody < control->Evolve.iNumBodies; iBody++) {
    ...
    if (body[iBody].bFlareBin) {
      AddModuleFlareBin(control, module, iBody, iModule);
  ...
  if (lTmp[0] >= 0) {
    ...
    */
    ...
    for (iModule = 0; iModule < iNumIndices; iModule++) {
      ...
      } else if (strcmp(sLower(saTmp[iModule]), "flarebin") == 0) {
        body[iFile - 1].bFlareBin = 1;
        module->iBitSum[iFile - 1] += FLAREBIN;
...
void PrintModuleList(FILE *file, int iBitSum, int bPadString) {
  ...
  if (iBitSum & FLAREBIN) {
    ...
    space++;
    fprintf(file, "FLAREBIN");
    nspaces -= strlen("FLAREBIN");
  }
  if (iBitSum & GALHABIT) {
    if (space) {
      fprintf(file, " ");
