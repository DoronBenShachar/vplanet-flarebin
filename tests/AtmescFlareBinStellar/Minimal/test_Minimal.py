...
INFILE = "tests/AtmescFlareBinStellar/Minimal/vpl.in"
LOG_PATH = REPO_ROOT / "Minimal.log"
STAR_PATH = REPO_ROOT / "Minimal.star.forward"
PLANET_PATH = REPO_ROOT / "Minimal.planet.forward"


def _run_case():
    for path in (LOG_PATH, STAR_PATH, PLANET_PATH):
        if path.exists():
            path.unlink()

    subprocess.run([str(BIN), INFILE], cwd=REPO_ROOT, check=True)

    star = np.atleast_2d(np.loadtxt(STAR_PATH))
    planet = np.atleast_2d(np.loadtxt(PLANET_PATH))
